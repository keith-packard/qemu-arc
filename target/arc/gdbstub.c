 ...
 */
static int arcv3_get_reg(const ARCCPU *cpu, GByteArray *mem_buf, target_ulong n)
{
    if (cpu->family & ARC_OPCODE_V3_ARC32) {
        return gdb_get_reg32(mem_buf, n);
    } else if (cpu->family & ARC_OPCODE_V3_ARC64) {
        return gdb_get_reg64(mem_buf, n);
    } else {
        g_assert_not_reached();
    }
}

static target_ulong arcv3_load_mem(const ARCCPU *cpu, const void *mem_buf)
{
    if (cpu->family & ARC_OPCODE_V3_ARC32) {
        return ldl_p(mem_buf);
    } else if (cpu->family & ARC_OPCODE_V3_ARC64) {
        return ldq_p(mem_buf);
    } else {
        g_assert_not_reached();
    }
}
...
int gdb_v3_core_read(CPUState *cs, GByteArray *mem_buf, int n)
{
    ...
    return arcv3_get_reg(cpu, mem_buf, regval);
}
...
int gdb_v3_core_write(CPUState *cs, uint8_t *mem_buf, int n)
{
    ARCCPU *cpu = ARC_CPU(cs);
    CPUARCState *env = &cpu->env;
    target_ulong regval = arcv3_load_mem(cpu, mem_buf);
...
void arc_cpu_register_gdb_regs_for_features(ARCCPU *cpu)
{
    ...
    if (cpu->family & ARC_OPCODE_ARCV2) {
        gdb_register_coprocessor(cs,
          ...
          /* pos. in g packet */ 0);
    } else if (cpu->family & ARC_OPCODE_V3_ARC32) {
        gdb_register_coprocessor(cs,
                                 gdb_v3_aux_read,
                                 gdb_v3_aux_write,
                                 GDB_ARCV3_AUX_LAST,
